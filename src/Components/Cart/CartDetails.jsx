...
const CartDetails = () => {
  const { toggleCart, cart, deleteCartItem  ,  updateCartItemQuantity} = useContext(MyContaxt);
  ...
  return (
    <>
      {toggleCart && (
        <div className="w-full md:px-2 fixed  md:w-1/4 h-full absolute top-30 right-0 bg-[#F9F9F9]">
            ...
            {cart.map((item) => (
              ...
              >
                <div className="flex items-center px-2 justify-between">
                  <p  onClick={() => updateCartItemQuantity(item.id)}   className="text-black p-1 px-2 font-semibold cursor-pointer  flex justify-center rounded-full items-center bg-[#FC8A06]">
          ...
          <div className="flex mx-1 rounded-2xl px-4 mt-7 py-2 h-[50px] bg-[#FC8A06] justify-between items-center">
            ...
            <p className="text-2xl font-bold text-white">₹{totalPayable}.00</p>
          </div>
          <div className="flex flex-col mt-5 space-y-4 items-center justify-between">
            <p className="text-1xl md:text-2xl border-2 w-[93%] py-2 flex items-center justify-center rounded-3xl">Choose your free item<span className="ml-4 md:ml-1 text-[#FC8A06CC]"><FaCircleArrowDown size={24} /></span></p>
            <p className="text-1xl md:text-2xl border-2 w-[93%] py-2 flex items-center justify-center rounded-3xl">Apply Coupon code here<span className="ml-4 md:ml-1 text-green-300"><BsArrowRightCircleFill size={24} /></span></p>
          </div>
        </div>
      )}
      <ToastContainer />
